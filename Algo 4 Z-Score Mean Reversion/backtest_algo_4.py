...
# Strategy Parameters
ALGO_NAME = "Algo_4_ZScore_Mean_Reversion"
WINDOW = 20
TP = 0.01
SL = 0.01
QTY = 100
...
def calculate_zscore(prices_series):
    if len(prices_series) < WINDOW:
        return None
    recent = prices_series[-WINDOW:]
    mean = np.mean(recent)
    std = np.std(recent)
    if std == 0: return 0
    return (prices_series[-1] - mean) / std
