...
def calculate_vwap(df_subset):
    if df_subset.empty: return 0
    return (df_subset["Close"] * df_subset["Volume"]).sum() / df_subset["Volume"].sum()
...
def run_single_backtest(filepath):
    ...
    for day, day_df in df.groupby("Date"):
        ...
        for i in range(len(day_df)):
            ...
            # VWAP Logic (Rolling Window for backtester)
            vwap = calculate_vwap(day_df.iloc[max(0, i-WINDOW+1):i+1])

            if position == 0:
                if price < vwap: # Raw Crossover as per algo 39.py
                    position = 1
                    entry_price = price
                    entry_time = time
                elif price > vwap:
                ...
                # Exit Logic: TP / SL from original script
                tp_pct = 0.005 # 0.5% as per original
                sl_pct = 0.01  # 1% as per original
                
                tp_price = entry_price * (1 + tp_pct if position == 1 else 1 - tp_pct)
                sl_price = entry_price * (1 - sl_pct if position == 1 else 1 + sl_pct)
                ...
                if (position == 1 and price >= tp_price) or (position == -1 and price <= tp_price):
                    ...
                    exit_flag = True
                    reason = "SL"
                elif (position == 1 and price > vwap) or (position == -1 and price < vwap):
                    exit_flag = True
