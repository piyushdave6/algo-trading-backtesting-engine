...
# Strategy Parameters
ALGO_NAME = "Algo_20_RegimeSwitcher"
WINDOW = 20
TP = 0.005
SL = 0.01
QTY = 100
        ...
        for i in range(len(day_df)):
            ...
            if waiting_for_warmup:
                ...
                if counter < WARMUP:
                    ...
                    base_atr = calculate_atr(prices[-WINDOW:])

            # Regime Switcher Logic
            curr_atr = calculate_atr(prices[-WINDOW:])
            sma = np.mean(prices[-WINDOW:])
            
            # Simple SuperTrend (HL2 +/- ATR)
            hl2 = (max(day_df.iloc[max(0, i-WINDOW+1):i+1]['High']) + min(day_df.iloc[max(0, i-WINDOW+1):i+1]['Low'])) / 2
            st_upper = hl2 + 3 * curr_atr
            st_lower = hl2 - 3 * curr_atr

            if position == 0:
                if curr_atr > base_atr:
                    # REVERSION MODE
                    if price < sma * 0.995: # 0.5% below SMA
                        position = 1
                        entry_price = price
                        entry_time = time
                    elif price > sma * 1.005: # 0.5% above SMA
                        position = -1
                        entry_price = price
                        entry_time = time
                else:
