...
def calculate_vama(prices, prev_vama):
    if len(prices) < WINDOW:
        return prices[-1], 0.1 # default
    ...
    # Scale CV to alpha [0.02, 0.20]
    # Original algo 30.py uses a ratio that effectively keeps alpha at 0.20
    alpha = 0.20 
    
    vama = alpha * prices[-1] + (1 - alpha) * prev_vama
    return vama, alpha
...
def run_single_backtest(filepath):
    ...
    for day, day_df in df.groupby("Date"):
        ...
        for i in range(len(day_df)):
            ...
            vama, alpha = calculate_vama(prices, vama_prev)
            vama_prev = vama
            ...
            if position == 0:
                ...
                if (position == 1 and price >= tp_price) or (position == -1 and price <= tp_price):
                    ...
                    exit_flag = True
                    reason = "SL"
                elif (position == 1 and price < vama) or (position == -1 and price > vama):
                    exit_flag = True
                    reason = "VAMA Cross Exit"
