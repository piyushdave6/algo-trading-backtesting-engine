...
def calculate_vroc(df, window):
    vroc = df['Volume'].pct_change(periods=window) * 100
    return vroc
...
def run_single_backtest(filepath):
    ...
    for day, day_df in df.groupby("Date"):
        ...
        vroc_series = calculate_vroc(day_df, WINDOW)
        ...
        for i in range(len(day_df)):
            ...
            # VROC Logic
            curr_vroc = vroc_series[i]
            prev_price = day_df.iloc[i-1]["Close"]

            if position == 0:
                if curr_vroc > VROC_THRESHOLD:
                    if price > prev_price:
                        position = 1
                        entry_price = price
                        entry_time = time
