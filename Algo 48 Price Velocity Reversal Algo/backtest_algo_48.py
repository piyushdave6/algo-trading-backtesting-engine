...
# Strategy Parameters
ALGO_NAME = "Algo_48_VelocityReversal"
WINDOW = 5
TP = 0.01
SL = 0.01
QTY = 100
...
def run_single_backtest(filepath):
    ...
    for day, day_df in df.groupby("Date"):
        ...
        for i in range(len(day_df)):
            ...
            vel = calculate_velocity(prices[-WINDOW:])
            ...
            # Velocity Reversal Logic as per original script
            velocity = prices[-1] - prices[-2] if len(prices) >= 2 else 0
            prev_velocity = prices[-2] - prices[-3] if len(prices) >= 3 else 0
            acceleration = velocity - prev_velocity
            
            threshold = 2.0 # From original script
            
            if position == 0:
                # LONG: selling exhaustion (Vel < -2 and Accel > 0)
                if velocity < -threshold and acceleration > 0:
                    position = 1
                    entry_price = price
                    entry_time = time
                # SHORT: buying exhaustion (Vel > 2 and Accel < 0)
