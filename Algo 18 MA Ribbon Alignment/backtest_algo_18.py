...
def run_single_backtest(filepath):
    ...
    for day, day_df in df.groupby("Date"):
        ...
        # Calculate MAs
        ma_fast = day_df["Close"].rolling(window=FAST).mean()
        ma_mid = day_df["Close"].rolling(window=MID).mean()
        ma_slow = day_df["Close"].rolling(window=SLOW).mean()
        ...
        for i in range(len(day_df)):
            ...
            # Ribbon Logic
            f, m, s = ma_fast[i], ma_mid[i], ma_slow[i]
            ...
            if position == 0:
                ...
                if (position == 1 and price >= tp_price) or (position == -1 and price <= tp_price):
                    ...
                    exit_flag = True
                    reason = "SL"
                elif (position == 1 and f < m) or (position == -1 and f > m):
                    exit_flag = True
                    reason = "Alignment Break"
