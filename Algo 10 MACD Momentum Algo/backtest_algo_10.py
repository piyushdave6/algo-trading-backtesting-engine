...
def run_single_backtest(filepath):
    ...
    # Pre-calculate MACD for the whole day to speed up? 
    # No, we need to handle it tick-by-tick for consistency with warmup logic.
    # But for MACD, we can pre-calculate and then access.
    ...
    for day, day_df in df.groupby("Date"):
        ...
        # Calculate MACD for the day
        ema12 = day_df["Close"].ewm(span=EMA_SHORT, adjust=False).mean()
        ema26 = day_df["Close"].ewm(span=EMA_LONG, adjust=False).mean()
        ...
        for i in range(len(day_df)):
            ...
            if position == 0:
                ...
                # Logic Exit: Opposite crossover
                if not exit_flag:
                    if position == 1 and curr_macd < curr_signal:
                        exit_flag = True
                        reason = "MACD Cross-Exit"
                    elif position == -1 and curr_macd > curr_signal:
                        exit_flag = True
                        reason = "MACD Cross-Exit"
