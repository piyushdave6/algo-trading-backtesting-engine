...
# Strategy Parameters
ALGO_NAME = "Algo_17_PivotBreakout"
WARMUP = 21
TP = 0.005
SL = 0.01
QTY = 100
...
def run_single_backtest(filepath):
    ...
    for day, day_df in df.groupby("Date"):
        ...
        for i in range(len(day_df)):
            ...
            high = day_df.iloc[i]["High"]
            low = day_df.iloc[i]["Low"]
            ...
            if waiting_for_warmup:
                ...
                if counter < WARMUP:
                    ...
                    # Calculate initial pivots from warmup period
                    h_w = day_df.iloc[:counter]["High"].max()
                    l_w = day_df.iloc[:counter]["Low"].min()
                    c_w = day_df.iloc[counter-1]["Close"]
                    pivots = calculate_pivots(h_w, l_w, c_w)

            # Pivot Breakout Logic
            p, r1, s1, r2, s2 = pivots
            prev_price = day_df.iloc[i-1]["Close"]

            if position == 0:
                # LONG: Cross above R1 or R2
                if (prev_price <= r1 and price > r1) or (prev_price <= r2 and price > r2):
                    position = 1
                    entry_price = price
                    entry_time = time
                # SHORT: Cross below S1 or S2
                elif (prev_price >= s1 and price < s1) or (prev_price >= s2 and price < s2):
                    position = -1
                    entry_price = price
                    entry_time = time
            else:
