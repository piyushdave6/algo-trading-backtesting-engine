...
# Strategy Parameters
ALGO_NAME = "Algo_6_MA_Crossover"
FAST_WINDOW = 5
SLOW_WINDOW = 12
THRESHOLD = 0.001 # 0.1% crossover threshold as per Doc
TP = 0.005
SL = 0.01
QTY = 100
WARMUP = 21

def calculate_ma(prices_series, window):
    if len(prices_series) < window:
        return None
    return np.mean(prices_series[-window:])
...
def run_single_backtest(filepath):
    ...
    for day, day_df in df.groupby("Date"):
        ...
        for i in range(len(day_df)):
            ...
            # Algo 6 Logic: MA Crossover
            ma_fast = calculate_ma(prices, FAST_WINDOW)
            ma_slow = calculate_ma(prices, SLOW_WINDOW)
            ...
            if position == 0:
                # LONG: Fast > Slow + THRESHOLD
                if diff > THRESHOLD:
                    position = 1
                    entry_price = price
                    entry_time = time
                # SHORT: Fast < Slow - THRESHOLD
                elif diff < -THRESHOLD:
                    position = -1
                    entry_price = price
                    entry_time = time
                ...
                # Option: Exit if crossover reverses (Logic Exit)
                if not exit_flag:
                    if (position == 1 and diff < 0) or (position == -1 and diff > 0):
                        exit_flag = True
                        reason = "Cross-Exit"
