...
def calculate_heikin_ashi(df):
    ...
    for i in range(1, len(df)):
        ha_open.append((ha_open[i-1] + ha_df.iloc[i-1]['HA_Close']) / 2)
...
def run_single_backtest(filepath):
    ...
    for day, day_df in df.groupby("Date"):
        ...
        ha_df = calculate_heikin_ashi(day_df)
        ...
        for i in range(len(day_df)):
            ...
            # Heikin-Ashi Logic
            is_green = ha_row['HA_Close'] > ha_row['HA_Open']
            is_red = ha_row['HA_Close'] < ha_row['HA_Open']
            ...
            if position == 0:
                ...
                if (position == 1 and price >= tp_price) or (position == -1 and price <= tp_price):
                    ...
                    exit_flag = True
                    reason = "SL"
                elif (position == 1 and is_red) or (position == -1 and is_green):
                    exit_flag = True
                    reason = "HA Color Flip"
