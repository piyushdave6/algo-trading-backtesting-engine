...
# Strategy Parameters
ALGO_NAME = "Algo_3_Gradient_Convergence"
LOOKBACK = 5  # Fair Value window as per Doc
TP = 0.01  # 1% as per HMA Standardization
SL = 0.01  # 1% as per HMA Standardization
QTY = 100
WARMUP = 21

def calculate_fair_value(prices_series):
    if len(prices_series) < LOOKBACK:
        return None
    return np.mean(prices_series[-LOOKBACK:])
...
def run_single_backtest(filepath):
    ...
    for day, day_df in df.groupby("Date"):
        ...
        for i in range(len(day_df)):
            ...
            # Algo 3 Logic: Mean Reversion
            fair_value = calculate_fair_value(prices)
            if fair_value is None:
                equity_curve.append({"Datetime": time, "Daily_Equity": day_capital, "Cumulative_PnL": cumulative_pnl})
                continue

            if position == 0:
                # LONG: price < fair_value
                if price < fair_value:
                    position = 1
                    entry_price = price
                    entry_time = time
                # SHORT: price > fair_value
                ...
                # Option: Exit if price crosses fair value (Mean Reversion)
                if not exit_flag:
                    if (position == 1 and price >= fair_value) or (position == -1 and price <= fair_value):
                        exit_flag = True
