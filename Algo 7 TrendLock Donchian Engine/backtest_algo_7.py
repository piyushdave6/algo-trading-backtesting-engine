...
# Strategy Parameters
ALGO_NAME = "Algo_7_TrendLock_Donchian"
WINDOW = 10
RANGE_FILTER = 0.03 # 3%
TP = 0.005
SL = 0.02
...
def calculate_donchian(prices_series):
    if len(prices_series) < WINDOW:
        return None, None
    upper = max(prices_series[-WINDOW:])
    lower = min(prices_series[-WINDOW:])
    return upper, lower
        ...
        for i in range(len(day_df)):
            ...
            if waiting_for_warmup:
                ...
                if counter < WARMUP:
                    ...
                    # On warmup end, lock the first channel
                    locked_upper, locked_lower = calculate_donchian(prices)
                    # Range check
                    if locked_upper and locked_lower:
                        if (locked_upper - locked_lower) / price > RANGE_FILTER:
                            # Reject and wait again (reset counter)
                            waiting_for_warmup = True
                            counter = WARMUP - 5 # wait a few more candles
                            continue

            if position == 0:
                if locked_upper is not None and locked_lower is not None:
                    # Breakout Logic
                    if price > locked_upper:
                        position = 1
                        entry_price = price
                        entry_time = time
                    elif price < locked_lower:
                        position = -1
                        entry_price = price
                        entry_time = time
            else:
                ...
                if exit_flag:
                    ...
                    position = 0
                    waiting_for_warmup = True # Standard HMA: wait after trade
                    counter = 0
